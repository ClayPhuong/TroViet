...
//    public HoaDon get(Long id) throws SQLException {
//        Statement statement = (Statement) con.createStatement();
//        String query = "select top 1 * from HoaDon where IdHopDong = ? and Trash!=0 order by NgayKetThuc desc;" + id;
//        ResultSet rs = ((java.sql.Statement) statement).executeQuery(query);
//        if (rs.next()) {
//           HoaDon hoaDon = getFromResultSet(rs);
...
//    }
    ...
    public HoaDon get(Long idHopDong) throws SQLException {
    	String query = "select top 1 * from HoaDon where IdHopDong = ? and trash!=0 and Status=0;";
    	PreparedStatement ps = con.prepareStatement(query);
        ps.setLong(1, idHopDong);
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
    ...
    public int updateByID(HoaDon hdon) throws SQLException {
            ...
            if (ps.executeUpdate() == 0) {
            	throw new SQLException("lỗi khi update");
			}
        return -1;
    }
